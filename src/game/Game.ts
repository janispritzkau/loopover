...
export class Game {
  ...
  scramble() {
    scrambleBoard(this.board, this.noRegrips ? this.active : undefined)
  }
  ...
  render() {
    ...
    for (let i = 0; i < (this.moveAxis == Axis.Col ? this.cols : this.rows); i++) {
      ...
      for (let j = Math.floor(-moveAmount); j < w - Math.floor(moveAmount); j++) {
        ...
        if ((this.noRegrips || this.highlightActive) && index == this.active) {
  ...
  private onTouchStart = (identifier: number, pointer: Pointer) => {
    ...
    if (!(this.noRegrips && !this.board.isSolved())) this.active = this.board.grid[pointer.startRow][pointer.startCol]
  }
